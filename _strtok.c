...
/**
 * t_strl - returns token's string lgth for mallocing
 * @str: a token
 * @pos: index position in user's command typed into shell
 * @delm: delimeter (e.g. " ");
 * Return: token lgth
 */
int t_strl(char *str, int pos, char delm)
{
	int l = 0;
...
{
	int buffsize = 0, p = 0, si = 0, i = 0, l = 0, se = 0, t = 0;
	char **toks = NULL, d_ch;
	...
	while (str[se] != '\0')	/* find string ending index */
		se++;
	while (si < se)
	{ /* malloc lgths for each token ptr in array */
		if (str[si] != d_ch)
		{
			l = t_strl(str, si, d_ch);
			toks[p] = malloc(sizeof(char) * (l + 1));
			if (toks[p] == NULL)
				return (NULL);
			i = 0;
